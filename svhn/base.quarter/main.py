    ...
    def _build_graph(self, inputs):
        ...
        with argscope(Conv2D, nl=BNReLU, use_bias=False):
            logits = (LinearWrap(image)
                      .Conv2D('conv1', ch(24), 5, padding='VALID')
                      .MaxPooling('pool1', 2, padding='SAME')
                      .Conv2D('conv2', ch(32), 3, padding='VALID')
                      .Conv2D('conv3', ch(32), 3, padding='VALID')
                      .MaxPooling('pool2', 2, padding='SAME')
                      .Conv2D('conv4', ch(64), 3, padding='VALID')
                      .Dropout('drop', 0.5)
                      .FullyConnected('fc0', ch(512),
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            InferenceRunner(data_test,
                            [ScalarStats('accuracy'), ScalarStats('cost'), ClassificationError()])
        ],
        max_epoch=350,
    )
