...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        mask = tf.Variable('mask', [1, 0, 0, 0])

        def proc(x, cnt=[0]):
            x = x * mask[cnt[0]] + tf.stop_gradient(x) * (1 - mask[cnt[0]])
            cnt[0] += 1
            return x

        image = image * 2 - 1   # center the pixels values at zero
        x = image
        x = Conv2D('conv0', x, 32, 3)
        x = proc(x)
        x = MaxPooling('pool0', x, 3, stride=2, padding='VALID')
        x = Conv2D('conv1', x, 32, 3)
        x = proc(x)
        x = Conv2D('conv2', x, 32, 3)
        x = proc(x)
        x = MaxPooling('pool1', x, 3, stride=2, padding='VALID')
        x = Conv2D('conv3', x, 32, 3)
        x = proc(x)
